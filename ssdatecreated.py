...
for item in slideshow_id:
	for i in range(1, 16):
		...
		print "id = " + str(i) +", date created = " + str(y.slideshow.created.text)
	...
	# with con:
	# 	cur = con.cursor()
	# 	cur.execute("UPDATE slideshare_quickbase SET Date_Slideshow_Created = '" +str(y.slideshow.created.text)+ "'WHERE id='"+ i + "'")
	# 	print "id = " + str(i) +"date created = " + "" str(y.slideshow.created.text)
print "Done"																												
